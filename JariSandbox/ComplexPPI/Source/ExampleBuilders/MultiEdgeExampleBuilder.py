            ...
            for j in range(i+1,len(sentenceGraph.tokens)):
                ...
                if len(path) > 1:#> 2:
                    ...
                    if sentenceGraph.interactionGraph.has_edge(path[-1], path[0]):
                        intEdges = sentenceGraph.interactionGraph.get_edge(path[-1], path[1])
                        for intEdge in intEdges:
                            categoryName = intEdge.attrib["type"]
        ...
        for i in range(1,len(pathTokens)-1):
            if len(pathEdges[i-1][i]) > 0 and len(pathEdges[i][i+1]) > 0:
                for pathEdge in pathEdges[i-1][i]:
                    features[self.featureSet.getId("depRight1_"+pathEdge.attrib["type"])] = 1
                for pathEdge in pathEdges[i][i+1]:
                    features[self.featureSet.getId("depRight2_"+pathEdge.attrib["type"])] = 1
            ...
            if len(pathEdges[i][i-1]) > 0 and len(pathEdges[i+1][i]) > 0:
                for pathEdge in pathEdges[i][i-1]:
                    features[self.featureSet.getId("depLeft1_"+pathEdge.attrib["type"])] = 1
                for pathEdge in pathEdges[i+1][i]:
                    features[self.featureSet.getId("depLeft2_"+pathEdge.attrib["type"])] = 1
            ...
            if len(pathEdges[i][i-1]) > 0 and len(pathEdges[i][i+1]) > 0:
                for pathEdge in pathEdges[i][i-1]:
                    features[self.featureSet.getId("depTop1_"+pathEdge.attrib["type"])] = 1
                for pathEdge in pathEdges[i][i+1]:
                    features[self.featureSet.getId("depTop2_"+pathEdge.attrib["type"])] = 1
            ...
            if len(pathEdges[i-1][i]) > 0 and len(pathEdges[i+1][i]) > 0:
                for pathEdge in pathEdges[i-1][i]:
                    features[self.featureSet.getId("depBottom1_"+pathEdge.attrib["type"])] = 1
                for pathEdge in pathEdges[i+1][i]:
                    features[self.featureSet.getId("depBottom2_"+pathEdge.attrib["type"])] = 1
