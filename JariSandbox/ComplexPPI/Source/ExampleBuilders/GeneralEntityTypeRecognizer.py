    ...
    def buildExamples(self, sentenceGraph):
        ...
        for i in range(len(sentenceGraph.tokens)):
            ...
            if sentenceGraph.tokenIsEntityHead[token] != None:
                category = self.classSet.getId(sentenceGraph.tokenIsEntityHead[token].attrib["type"])
            else:
                category = 1
            ...
            if i > 0:
                ...
                features[self.featureSet.getId("linear_-1_POS_"+t.attrib["POS"])] = 1
                if sentenceGraph.tokenIsName[t]:
                    features[self.featureSet.getId("linear_-1_isName")] = 1
            if i > 1:
                t = sentenceGraph.tokens[i-2]
                features[self.featureSet.getId("linear_-2_txt_"+sentenceGraph.getTokenText(t))] = 1
