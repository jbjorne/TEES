...
def makeOneClassExamples(input, output, className, classSet):
    print "Converting file", input, "for trigger class", className
    
    classId = str(classSet.getId(className, False))
    classMap = {}
    for name in classSet.getNames():
        if name.find(className) != -1:
            classMap[classSet.getId(name, False)] = True
        else:
            classMap[classSet.getId(name, False)] = False
    
    f = open(input, "rt")
    outF = open(output, "wt")
    for line in f:
        classPart, rest = line.split(" ", 1)
        if classMap[int(classPart)]:
            outF.write(classId + " " + rest)
        else:
            outF.write("1 " + rest)
    f.close()
    outF.close()
...
for triggerClass in triggerClasses:
    ...
    if "local" not in options.csc:
        ...
        if "louhi" in options.csc:
            c = CSCConnection(CSC_WORKDIR+"/trigger-models-"+triggerClass, "jakrbj@louhi.csc.fi", clear)
        else:
            c = CSCConnection(CSC_WORKDIR+"/trigger-models-"+triggerClass, "jakrbj@murska.csc.fi", clear)
    else:
        c = None
