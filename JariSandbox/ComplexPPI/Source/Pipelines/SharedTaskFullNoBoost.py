# Builds genia trigger and edge example files for a specified parse
...
optparser.add_option("-p", "--parse", default="split-McClosky", dest="parse", help="")
optparser.add_option("-t", "--tokenization", default="split-McClosky", dest="tokenization", help="")
optparser.add_option("-n", "--name", default="SharedTaskFullNoBoost", dest="name", help="experiment name")
(options, args) = optparser.parse_args()

WORKDIR = "/usr/share/biotext/GeniaChallenge/extension-data/genia/" + options.name
...
TRAIN_AND_DEVEL_FILE=CORPUS_DIR + "everything.xml"
...
TRIGGER_CLASS_NAMES="genia-trigger-ids.class_names"
...
EDGE_CLASS_NAMES="genia-edge-ids.class_names"
...
if True:
    ...
    c = CSCConnection("GeniaChallenge/" + options.name + "/parameters/triggers-"+PARSE, "jakrbj@murska.csc.fi", True, memory=8388608)
...
if True:
    ...
    # The optimize-function takes as parameters a Classifier-class, an Evaluator-class
    # and input and output files
    c = CSCConnection("GeniaChallenge/" + options.name + "/parameters/edges-"+PARSE, "jakrbj@murska.csc.fi", True, memory=8388608)
...
if True:
    ...
    MultiEdgeExampleBuilder.run("devel-predicted-triggers.xml", "edge-devel-examples-ptrig", PARSE, TOK, EDGE_FEATURE_PARAMS, "genia-edge-ids")
    ...
    ExampleUtils.writeToInteractionXML("edge-devel-examples-ptrig", "edge-devel-classifications-ptrig", "devel-predicted-triggers.xml", xmlFilename, "genia-edge-ids.class_names", PARSE, TOK)
    ix.splitMergedElements(xmlFilename, xmlFilename)
    ix.recalculateIds(xmlFilename, xmlFilename, True)
    print >> sys.stderr, "############################################"
    ...
    gifxmlToGenia(xmlFilename, "geniaformat", 2)
...
if False:
    ...
    ###############################################################################
    # Build edge examples on top of predicted triggers
    MultiEdgeExampleBuilder.run("test-predicted-triggers.xml", "edge-test-examples-with-pred-triggers", PARSE, TOK, EDGE_FEATURE_PARAMS, "genia-edge-ids")
