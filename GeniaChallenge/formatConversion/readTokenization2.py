...
def readDepTrees(depFile,options):
...
def readPOSTags(pennFile,options):
...
def computeTokens(txtFile,posTags,options):
    ...
    tokTxt=[t[1] for s in posTags for t in s] #read tokens
...
def reorderCorpus(cNode,options):
    ...
    for dNode in cNode:
        ...
        depTree=readDepTrees(options.depFile,options)
        posTags=readPOSTags(options.pennFile,options)
        rawTxt,charOffsets,sFinal=computeTokens(options.txtFile,posTags,options)
...
if __name__=="__main__":
    desc="Reads from stdin an interaction XML produced by geniaToGifxml.py and plugs in the tokenization and parses from .penn and .dep files. Writes to stdout."
    usage="python readTokenization.py [options] < in > out"
    parser = OptionParser(description=desc,usage=usage)
    parser.add_option("-x", "--txt", dest="txtFile", action="store", default=None, help="The raw txt file.", metavar="FILE.txt")
    parser.add_option("-d", "--dep", dest="depFile", action="store", default=None, help="The dep file.", metavar="FILE.dep")
    parser.add_option("-p", "--penn", dest="pennFile", action="store", default=None, help="The penn file.", metavar="FILE.penn")
