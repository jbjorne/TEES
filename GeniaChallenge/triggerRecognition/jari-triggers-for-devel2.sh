...
python $PIPELINE -b GeneralEntityTypeRecognizer -x "style:typed" -c SVMMultiClassClassifier -e AveragingMultiClassEvaluator -y $CLASSIFIER_PARAMS -i $TRAIN_FILE -s $TEST_FILE -o $HOME/cvs_checkout/GeniaChallenge/triggerRecognition/debug -m $HOME/cvs_checkout/GeniaChallenge/triggerRecognition/debug/finalout.xml -p split-Charniak-Lease
