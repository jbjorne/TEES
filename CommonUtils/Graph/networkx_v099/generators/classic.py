...
def grid_graph(dim,periodic=False):
    ...
    """    
    from networkx_v099.utils import is_list_of_ints
    dlabel="%s"%dim
    if dim==[]:
        G=networkx_v099.Graph()
    ...
    if not is_list_of_ints(dim):
        raise networkx_v099.NetworkXError,"dim is not a list of integers"
    if min(dim)<=0:
        raise networkx_v099.NetworkXError,\
    ...
    while len(dim)>0:
        ...
        Gold=G.copy()
        G=networkx_v099.operators.cartesian_product(Gnew,Gold)
    # graph G is done but has labels of the form (1,(2,(3,1)))
    # so relabel
    H=networkx_v099.operators.relabel_nodes(G, networkx_v099.utils.flatten)
