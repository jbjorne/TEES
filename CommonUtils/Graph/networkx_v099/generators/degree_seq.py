...
import random
import networkx
import networkx_v099.utils
from networkx_v099.generators.classic import empty_graph
import heapq
#---------------------------------------------------------------------------
...
def configuration_model(deg_sequence,seed=None):
    ...
    >>> from networkx_v099.utils import powerlaw_sequence
    >>> z=nx.create_degree_sequence(100,powerlaw_sequence)
    >>> G=nx.configuration_model(z)

    The pseudograph G is a networkx_v099.MultiGraph that allows multiple (parallel) 
    ...
    N=len(deg_sequence)
#    G=networkx_v099.empty_graph(N,create_using=networkx_v099.Graph()) # no multiedges or selfloops

    # allow multiedges and selfloops
    G=networkx_v099.empty_graph(N,create_using=networkx_v099.MultiGraph())
...
def expected_degree_graph(w, seed=None):
    ...
    n = len(w)
    # allow self loops
    G=networkx_v099.empty_graph(n,create_using=networkx_v099.Graph())
...
def havel_hakimi_graph(deg_sequence):
    ...
    N=len(deg_sequence)
    G=networkx_v099.empty_graph(N) # always return a simple graph
...
def is_valid_degree_sequence(deg_sequence):
    ...
    if not networkx_v099.utils.is_list_of_ints(deg_sequence):
...
def create_degree_sequence(n, sfunction=None, max_tries=50, **kwds):
    ...
    see networkx_v099.Utils.

    >>> from networkx_v099.utils import uniform_sequence
...
def double_edge_swap(G, nswap=1):
    ...
    cdf=networkx_v099.utils.cumulative_distribution(deg.values())  # cdf of degree
    if len(cdf)<4:
        raise networkx_v099.NetworkXError, \
...
#        if random.random() < 0.5: continue # trick to avoid periodicities?
        ...
        (ui,xi)=networkx_v099.utils.discrete_sequence(2,cdistribution=cdf) 
...
def connected_double_edge_swap(G, nswap=1):
    ...
    """
    import math
    if not networkx_v099.is_connected(G):
       raise networkx_v099.NetworkXException("Graph not connected")
    ...
    dk=deg.keys() # key labels 
    ideg=G.degree()
    cdf=networkx_v099.utils.cumulative_distribution(G.degree()) 
    if len(cdf)<4:
        raise networkx_v099.NetworkXError, \
          "Graph has less than four nodes."
    window=1
    while n < nswap:
        ...
        while wcount < window and n < nswap:
            ...
            n+=1
            wcount+=1
        if networkx_v099.is_connected(G): # increase window
            window+=1
    ...
    while O or selected:
        ...
        if a_list[i] and j in b_set:
            ...
            if db == (2*bsize - wa):
                ...
                raise networkx_v099.NetworkXError, "THIS SHOULD NOT HAPPEN! - not graphable"
                continue
