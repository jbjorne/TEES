...
def adj_matrix(G,nodelist=None):
    ...
    This just calls networkx_v099.convert.to_numpy_matrix.

    If you want a pure python adjacency matrix represntation try
    networkx_v099.convert.to_dict_of_dicts with weighted=False,
    which will return a dictionary-of-dictionaries format that
    can be addressed as a sparse matrix.

    """
    return networkx_v099.to_numpy_matrix(G,nodelist=nodelist)
...
def laplacian(G,nodelist=None):
    ...
    n=G.order()
    I=N.identity(n)
    A=N.asarray(networkx_v099.to_numpy_matrix(G,nodelist=nodelist))
    D=I*N.sum(A,axis=1)
    L=D-A
    return L
