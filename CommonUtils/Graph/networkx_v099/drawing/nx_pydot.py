...
Import and export Networkx_v099 graphs in Graphviz dot format using pydot.
...
import sys
from networkx_v099.utils import _get_fh
try:
    import pydot
except ImportError:
    raise
...
def from_pydot(P):
    """Return a Networkx_v099 Graph or DiGraph from a pydot graph.
    ...
    if P.get_type()=='graph': # undirected
        if P.get_strict(None):
            create_using=networkx_v099.Graph()
        else:
            create_using=networkx_v099.MultiGraph()
    else:
        if P.get_strict(None):
            create_using=networkx_v099.DiGraph()
        else:
            create_using=networkx_v099.MultiDiGraph()

    # assign defaults        
    N=networkx_v099.empty_graph(0,create_using)
    N.name=P.get_name()
    node_attr={}
...
def to_pydot(N, graph_attr=None, node_attr=None, edge_attr=None,
              strict=True):
    """Return a pydot graph from a Networkx_v099 graph N.
...
def pydot_from_networkx(N):
	"""Creates a pydot graph from an networkx_v099 graph N"""
        from warnings import warn
        warn('pydot_from_networkx_v099 is replaced by to_pydot', DeprecationWarning)
        return to_pydot(N)

def networkx_from_pydot(D, create_using=None):
	"""Creates an networkx_v099 graph from an pydot graph D"""
        from warnings import warn
...
def pydot_layout(G,prog='neato',root=None, **kwds):
    ...
    """
    from networkx_v099.drawing.nx_pydot import pydot_from_networkx
    try:
        import pydot
    except:
