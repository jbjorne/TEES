"""
Convert Networkx_v099 graphs to and from other formats.
...
For graphviz formats see networkx_v099.drawing.nx_pygraphviz
or networkx_v099.drawing.nx_pydot.
...
def _prep_create_using(create_using):
    ...
    If create_using is None return the default (just networkx_v099.Graph())
    If create_using.clear() works, assume it returns a graph object.
    Otherwise raise an exception because create_using is not a networkx_v099 graph.
...
def from_whatever(thing,create_using=None,multigraph_input=False):
    """Attempt to make a Networkx_v099 graph from an known type.

    Current known types are:

       any Networkx_v099 graph
       dict-of-dicts
       dist-of-lists
       numpy matrix
       numpy ndarray
    ...
    if hasattr(thing,"adj"):
        try:
            ...
            raise networkx_v099.NetworkXError,\
                  "Input is not a correct Networkx_v099 graph."

    # pygraphviz  agraph
    if hasattr(thing,"is_strict"):
        try:
            return networkx_v099.from_agraph(thing,create_using=create_using)
        except:
            raise networkx_v099.NetworkXError,\
...
def from_dict_of_lists(d,create_using=None):
    """Return a Networkx_v099 graph G from a Python dict of lists.
...
def from_dict_of_dicts(d,create_using=None,multigraph_input=False):
    """Return a Networkx_v099 graph G from a Python dictionary of dictionaries.

    The value of the inner dict becomes the edge_data for the Networkx_v099 graph.
...
def from_numpy_matrix(A,create_using=None):
    """Return networkx_v099 graph G from numpy matrix adjacency list. 
...
def from_scipy_sparse_matrix(A,create_using=None):
    """Return networkx_v099 graph G from scipy scipy sparse matrix
    adjacency list. 
...
if __name__ == "__main__":
    ...
    if sys.version_info[:2] < (2, 4):
        ...
        sys.exit(-1)
    # directory of networkx_v099 package (relative to this)
