...
def graph_could_be_isomorphic(G1,G2):
    ...
    t1=networkx_v099.triangles(G1,with_labels=True)
    c1=networkx_v099.number_of_cliques(G1,with_labels=True)
    props1=[ [d1[v], t1[v], c1[v]] for v in d1 ]
    props1.sort()
    
    d2=G2.degree(with_labels=True)
    t2=networkx_v099.triangles(G2,with_labels=True)
    c2=networkx_v099.number_of_cliques(G2,with_labels=True)
...
def is_isomorphic(G1,G2):
    ...
    Uses the vf2 algorithm - see networkx_v099.isomorphvf2
        
    """
    if G1.directed and G2.directed:
        return networkx_v099.DiGraphMatcher(G1,G2).is_isomorphic()
    elif not (G1.directed and G2.directed):
        return networkx_v099.GraphMatcher(G1,G2).is_isomorphic()
    else:
