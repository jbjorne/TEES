...
# A unique global instance of the PorterStemmer class, so that
# a new object does not need to be created every time something
# needs to be stemmed.
...
def stem(word, startPos=None, endPos=None):
    ...
    Return value (string):
        The stem 
    """
    global porterStemmerSingleton
    if porterStemmerSingleton == None:
        porterStemmerSingleton = PorterStemmer()
    
    if startPos == None:
        startPos = 0
    if endPos == None:
        endPos = len(word) - 1
    return porterStemmerSingleton.stem(word, startPos, endPos)
